...
def calc_benjamini_hochberg_cutoff(p_values, fdr, numTests):
    ...
    return(lastP)
#    for k, p_value in enumerate(p_values):  # k starts at 0 (?)
#        if (p_value > (float(k + 1) / float(numTests)) * fdr):
#            return ((float(k + 1) / float(numTests)) * fdr)
#        else:
#            lastP = p_value
#    return (lastP)
...
def parseResults(param, stri, mode):
    ...
    ps  = [(z.split("\t"))[23] for z in txt][1:]  # FBAT column 23 is the p-values
    ps  = [safeFloat(z) for z in ps]
    psi = [(z.split("\t"))[25] for z in txt][1:]  # CIFBAT column 25 is the p-value interval.
    psa = [ciFun(z,"cifbat-interval") for z in psi]   # column 25 is the p-value interval.

    pvalCutOff = calc_benjamini_hochberg_cutoff(sorted(ps), param["cutoff"], len(ps))  # FDR based on FBAT #

    for i in range(0,len(psi)):
        #if ps[i] > pvalCutOff and psa[i] <= pvalCutOff:
        #if ps[i] > psa[i]:
        #    print(param["resultsDir"]+"/log_"+stri+".txt"+"\t"+ str(pvalCutOff)+"\t"+str(ps[i])+"\t"+str(psa[i]))
        #if ps[i] < pvalCutOff:
        #    print("**** " + param["resultsDir"]+"/log_"+stri+".txt"+"\t"+ str(pvalCutOff)+"\t"+str(ps[i])+"\t"+str(psa[i]))
        if ps[i] > pvalCutOff and psa[i] <= pvalCutOff:
            print(">>>> " + param["resultsDir"]+"/log_"+stri+".txt"+"\t"+ str(pvalCutOff)+"\t"+str(ps[i])+"\t"+str(psa[i]))
